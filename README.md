...
選片、篩選、排序，一行決策摘要直接給結論：

![排名清單與決策摘要](docs/screenshots/01-ranking.webp)

疊圖比較（同比例尺、每 5 公尺格線、1.7m 人形參照）——《奧德賽》在 IMAX GT 1.43:1 滿幅與杜比影院的成像差距，就是「銀幕大不等於畫面大」的實際樣子：

![疊圖比較四廳成像](docs/screenshots/02-overlay.webp)

展開「規格與依據」才看得到逐項得分、欄位級可信度標記與來源，不干擾第一層決策：

![規格與依據與逐項計分](docs/screenshots/03-evidence.webp)

行動版首屏：控制列選完收成一行摘要、疊圖預設收合。

<img src="docs/screenshots/04-mobile.webp" width="320" alt="行動版首屏">

> 截圖為 2026-08 的資料版本（深色主題），數值與廳數會隨資料維護更動。
...
- **2026 鑑別度片單**：精選《奧德賽》《極限返航》《曼達洛人與古古》《蜘蛛人：重生日》《沙丘：第三部》；只收不同畫幅、音效或特殊格式會影響選廳決策的年度商業大片，《沙丘》前兩部另列系列參考
...
- **影廳視覺識別**：display 字型聚焦主標與關鍵數字，片孔、幕簾只作低對比形狀語彙；尺寸與比例集中由面積數字、疊圖及展開規格呈現
- **決策摘要一行**：把排名翻成結論（「第 1 名是第 2 名的 2.1 倍」），隨篩選與排序即時更新
- **URL 狀態分享**：選片／篩選／排序／自選比較全部入網址，F5 不歸零、可傳給朋友；og 分享卡已就緒
- **行動版首屏**：控制列選完收成一行摘要、疊圖預設收合
- **外觀主題**：系統／深色／淺色三態；手動偏好保存在本機，切回系統會隨裝置即時更新
...
- [x] 體驗型廳與座位稽核（16）：MUCROWN、金鑽、GOLD CLASS、4DX、B.O.X.、A+、D-BOX 已按實體廳拆分；無法可靠對應的席次維持待確認
- [ ] 進行中：逐欄提升官方來源覆蓋（5）
